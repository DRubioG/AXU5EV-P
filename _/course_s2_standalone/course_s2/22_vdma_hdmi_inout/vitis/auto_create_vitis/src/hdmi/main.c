...
int main(void) {
	...
	i2c_init(&IicInstance0, XPAR_PSU_I2C_0_DEVICE_ID,100000);
	i2c_init(&IicInstance1, XPAR_PSU_I2C_1_DEVICE_ID,100000);
	...
	while(1){
		...
		Status = hpd_check(IicInstance0) ;
		if (Status){
			xil_printf("Hot plug detected\r\n");
			adv7511_init(IicInstance0) ;
			break;
		}
		sleep(1);
	};
...
void DemoPrintTest(u8 *frame, u32 width, u32 height, u32 stride, int pattern) {
	...
	//		break;
	case DEMO_PATTERN_0: //8 intervals color bar
