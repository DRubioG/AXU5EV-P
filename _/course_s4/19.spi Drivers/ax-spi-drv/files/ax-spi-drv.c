...
static int read_sr(struct ax_qflash_dev *flash)  
{  
    ssize_t retval;  
    u8 code = 0x05;  
    u8 val;  
    struct spi_device *spi = (struct spi_device *)flash->private_data;
...
static int wait_till_ready(struct ax_qflash_dev *flash)  
{  
    int count;  
    int sr;   
...
static ssize_t ax_flash_read(struct file *file, char __user *buf, size_t size, loff_t *offset)
{
    ...
    /* 读取数据 */
    ax_spi_read(dev, 0x00, b, 200 > size ? size : 200);
    
    /* 把读取到的数据拷贝到用户读取的地址 */
    ret = copy_to_user(buf, b, 200 > size ? size : 200);
    return 0;
}
...
static ssize_t ax_flash_write(struct file *file, const char __user *buf, size_t size, loff_t *offset)
{
    ...
    struct ax_qflash_dev *dev = (struct ax_qflash_dev *)file->private_data;
    /* 写入数据的buffer */
    static char user_data[200] = {0};
    int ret = 0;
    /* 获取用户需要发送的数据 */
    ret = copy_from_user(user_data, buf, 200> size ? size : 200);
    if(ret < 0)
    {
    ...
    /* 写入数据 */
    ax_spi_erase(dev, 0x00);
    mdelay(1000);
    ax_spi_write(dev, 0x00, user_data, 200 > size ? size : 200);
