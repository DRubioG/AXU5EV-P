...
int main(void)
{
    ...
    config_baseaddr = (u64)mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_SHARED, mem_fd, XPAR_PSU_GPIO_0_BASEADDR );
    if((u64)MAP_FAILED == config_baseaddr)
    {
        printf("mmap fail\r\n");
    }
    ...
    /*
         ...
         */
    XGpioPs_SetDirectionPin(Output_Pin, 1);
    XGpioPs_SetOutputEnablePin(Output_Pin, 1);

    while(1)
    {
        /* Set the GPIO output to be low. */
        XGpioPs_WritePin(Output_Pin, 0x0);
        usleep(500000);
        /* Set the GPIO Output to High. */
        XGpioPs_WritePin(Output_Pin, 0x1);
        msync((void *)config_baseaddr, 1024, MS_ASYNC);
        usleep(500000);
    }
