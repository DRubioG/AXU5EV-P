...
// clang-format off
    ...
    xf::cv::Mat<XF_8UC3, HEIGHT, WIDTH, XF_NPPC1> in_mat;

    xf::cv::Mat<XF_8UC3, NEWHEIGHT, NEWWIDTH,XF_NPPC1> out_mat;

// clang-format off
    #pragma HLS DATAFLOW
    // clang-format on

    xf::cv::Array2xfMat<INPUT_PTR_WIDTH, XF_8UC3, HEIGHT, WIDTH, XF_NPPC1>(img_inp, in_mat);
    xf::cv::resize<0, XF_8UC3, HEIGHT, WIDTH, NEWHEIGHT, NEWWIDTH, XF_NPPC1, 2>(in_mat, out_mat);
    xf::cv::xfMat2Array<OUTPUT_PTR_WIDTH, XF_8UC3, NEWHEIGHT, NEWWIDTH, NPC_T>(out_mat, img_out);
}
