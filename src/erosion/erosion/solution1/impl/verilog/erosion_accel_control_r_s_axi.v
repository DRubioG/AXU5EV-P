...
// ==============================================================
`timescale 1ns/1ps
module erosion_accel_control_r_s_axi
#(parameter
    ...
    output wire [63:0]                   process_shape,
...
localparam
    ...
    ADDR_PROCESS_SHAPE_DATA_0 = 6'h1c,
    ADDR_PROCESS_SHAPE_DATA_1 = 6'h20,
    ADDR_PROCESS_SHAPE_CTRL   = 6'h24,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_PROCESS_SHAPE_DATA_0: begin
                    rdata <= int_process_shape[31:0];
                end
                ADDR_PROCESS_SHAPE_DATA_1: begin
                    rdata <= int_process_shape[63:32];
                end
...
assign process_shape = int_process_shape;
...
// int_process_shape[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_process_shape[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PROCESS_SHAPE_DATA_0)
            int_process_shape[31:0] <= (WDATA[31:0] & wmask) | (int_process_shape[31:0] & ~wmask);
    end
end

// int_process_shape[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_process_shape[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PROCESS_SHAPE_DATA_1)
            int_process_shape[63:32] <= (WDATA[31:0] & wmask) | (int_process_shape[63:32] & ~wmask);
    end
end
