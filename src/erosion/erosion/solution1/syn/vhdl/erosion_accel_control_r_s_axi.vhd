...
entity erosion_accel_control_r_s_axi is
generic (
...
end entity erosion_accel_control_r_s_axi;
...
architecture behave of erosion_accel_control_r_s_axi is
    ...
    constant ADDR_PROCESS_SHAPE_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_PROCESS_SHAPE_DATA_1 : INTEGER := 16#20#;
    constant ADDR_PROCESS_SHAPE_CTRL   : INTEGER := 16#24#;
    ...
    signal int_process_shape   : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_PROCESS_SHAPE_DATA_0 =>
                        rdata_data <= RESIZE(int_process_shape(31 downto 0), 32);
                    when ADDR_PROCESS_SHAPE_DATA_1 =>
                        rdata_data <= RESIZE(int_process_shape(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    ...
    process_shape        <= STD_LOGIC_VECTOR(int_process_shape);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_PROCESS_SHAPE_DATA_0) then
                    int_process_shape(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_process_shape(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_PROCESS_SHAPE_DATA_1) then
                    int_process_shape(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_process_shape(63 downto 32));
                end if;
            end if;
        end if;
    end process;
